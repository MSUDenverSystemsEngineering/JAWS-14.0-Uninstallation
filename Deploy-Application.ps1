...
Try {
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'jfw,fsreader' -CloseAppsCountdown 60
		...
		If (Test-Path -Path "$envProgramFiles\Freedom Scientific Installation Information\356DE2A8-01EB-464e-9C33-0EEA3F923000-14.0" -PathType Container) {
			$exitCode = Execute-Process -Path "$envProgramFiles\Freedom Scientific Installation Information\356DE2A8-01EB-464e-9C33-0EEA3F923000-14.0\UninstallJAWS.exe" -Parameters "/Type silentRemoveShared" -WindowStyle "Hidden" -WaitForMsiExec -PassThru
			...
			If (Get-InstalledApplication -Name "Sentinel System Driver Installer") {
				$exitCode = Remove-MSIApplications -Name "Sentinel System Driver Installer" -PassThru
				If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
			}
			Get-ChildItem -Path "$envSystemDrive\Users" -Force | Where-Object { $_.PSIsContainer } | ForEach-Object {
				$desktopFolder = $_.FullName + "\Desktop"
				If (Test-Path $desktopFolder) {
					Remove-File -Path "$desktopFolder\JAWS 14.0.lnk"
					Remove-File -Path "$desktopFolder\FSReader 2.0.lnk"
				}
			}
		}
